    ...
    public class UserManager : IUserManager
    {
        ...
        public Operation<long> CreateUser(UserModel model)
        {
            ...
            return Operation.Create((Func<long>)(() =>
            {
                ...
                var roleId = _db.Query<Role>().Where((System.Linq.Expressions.Expression<Func<Role, bool>>)(r => r.RoleName == AppRole.Freelancer.ToString())).Select(r => r.RoleId).FirstOrDefault();
        ...
        public Operation<UserModel> FindUserById(long userId)
        {
            return Operation.Create(() =>
            {
                var user = _db.Query<User>(u => u.Profile ).Where( u => u.UserId == userId).FirstOrDefault();
                if (user == null) throw new Exception("User not Found");
        ...
        public Operation<UserModel[]> GetUsers()
        {
            return Operation.Create(() =>
            {
                var users = _db.Query<User>(u => u.Profile).ToList();
        ...
        public Operation<long> AddUser(UserModel model)
        {
            return Operation.Create((Func<long>)(() =>
            {
                ...
                //var roleId = _db.Query<Role>().Where(r => r.RoleName == AppRole.SystemAdministrator.ToString()).Select(r => r.RoleId).FirstOrDefault();
                //AddRoleToUser(model, roleId);
        ...
        public Operation<string> SetProfileImage(long userId, string mime, MemoryStream data)
        {
            return Operation.Create(() =>
            {
                var user = _db.Query<User>(u => u.Profile).FirstOrDefault(u => u.UserId == userId);
        ...
        //        new User() {UserId = 1, Email = "stephenajose@yahoo.com" },
        //        new User() { UserId = 1, Email = "stephenajose@yahoo.com" },
        //        new User() { UserId = 1, Email = "stephenajose@yahoo.com" },
        //        new User() { UserId = 1, Email = "stephenajose@yahoo.com" },
        //        new User() { UserId = 1, Email = "stephenajose@yahoo.com" }
