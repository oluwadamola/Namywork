...
module Stev.SystemAdmin.Controllers {
    ...
    class ServicesCtrl {
        ...
        constructor(_service, _notify, $uibModal) {
        ...
        addService() {
            ...
            instance.result.then(() => this.getServices());
        }
    }
    ...
    class AddServiceCtrl {
        ...
        constructor($uibModalInstance, _service, _notify, _storage) {
            this.instance = $uibModalInstance;
            this.service = _service;
            this.notify = _notify;
        ...
        getCategories() {
            this.service.getCategories().then(c => this.Categories = c);
        }

        getCities() {
            this.service.getCities().then(c => this.Cities = c);
        }

        Ok(form: angular.IFormController, service) {
            if (form.$valid) {
                ...
                this.service.addService(service).then(() => {
                    this.notify.success("Service is successfully created");
                    this.instance.close();
                });
            }
            else {
    ...
    class ServiceCtrl {
        ...
        constructor(_service, _user, $stateParams) {
            this.service = _service;
            this.user = _user;
            this.service.getService($stateParams["id"]).then(s => {
                this.Service = s;
                this.user.getUser(this.Service.UserId).then(u => {
                this.User = u;
                });
    ...
    module.controller("ServiceCtrl", ServiceCtrl);
    module.controller("AddServiceCtrl", AddServiceCtrl);
}